...
{
	cube->center.x = cube->tx;
	cube->center.y = cube->ty + cube->sfy;
	cube->center.z = cube->tz;

	cube->dim.x = 2.0 * cube->sfx;
	cube->dim.y = 2.0 * cube->sfy;
	cube->dim.z = 2.0 * cube->sfz;

	min->x = cube->center.x - 0.5*cube->dim.x;
	min->y = cube->center.y - 0.5*cube->dim.y;
	min->z = cube->center.z - 0.5*cube->dim.z;

	max->x = cube->center.x + 0.5*cube->dim.x;
	max->y = cube->center.y + 0.5*cube->dim.y;
	max->z = cube->center.z + 0.5*cube->dim.z;
